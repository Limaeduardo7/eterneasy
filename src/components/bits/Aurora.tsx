...
export function Aurora({
  colorStops = ["#102C47", "#C69A27", "#102C47"],
  amplitude = 1.2,
  blend = 0.6,
  speed = 0.6,
}: AuroraProps) {
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      const { speed: spd = 0.6, colorStops: cs, amplitude: amp, blend: bl } = propsRef.current;
      program.uniforms.uTime.value = t * 0.001 * spd * 0.1;
      program.uniforms.uAmplitude.value = amp ?? 1.2;
      program.uniforms.uBlend.value = bl ?? 0.6;
